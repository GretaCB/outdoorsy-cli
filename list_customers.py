...
if __name__ == "__main__":
    ...
    * Optionally sort the data by Vehicle type or by Full name
    ...
    if len(sys.argv) == 1:
        questions = [
            inquirer.Path("filepath", message="Which file would you like to import and list?",
                          exists=True, normalize_to_absolute_path=True, path_type=inquirer.Path.FILE),
            inquirer.List("sort", message="How would you like to sort the data in {filepath}?", choices=[
                          'No thanks', 'name', 'vehicle_type'])
        ]
    ...
    # This else block will cover --help and automated usage
        ...
        parser.add_argument(
            "file",
            # This includes builtin filepath validation
